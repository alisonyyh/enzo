...
export function AIRoutineGenerator({ questionnaireData, onComplete }: AIRoutineGeneratorProps) {
  ...
  useEffect(() => {
    // Reset completion flag when effect runs
    ...
    // Fire completion callback after animation
    const completionTimer = setTimeout(() => {
      if (hasCompletedRef.current) return;
      hasCompletedRef.current = true;
      console.log("AIRoutineGenerator: timer fired, calling onComplete");
      const routine = generateRoutine(questionnaireDataRef.current);
      onCompleteRef.current(routine);
    }, totalDuration);
  ...
  }, []); // Empty deps — run once on mount, use refs for current values
