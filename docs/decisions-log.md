...
| D47 | Notes Field in Bottom Sheet | **Multiline textarea below Activity Type grid, 200-char max, auto-grows to 3 lines** | Added a "Notes" field to the AddTaskFAB bottom sheet (both Add and Edit modes). For AI-generated tasks: **empty by default** — AI routine generation does not produce descriptions or commentary, so there is nothing to pre-populate (see D73). For custom tasks: empty unless user previously saved a note. For new tasks via FAB: empty with placeholder "Add a note...". Auto-grows up to 3 lines (72px), then scrolls. Max 200 characters with live character counter. Optional field — saving with empty notes is valid. Rationale: (1) Users want to log context ("accident near the back door") without a separate notes app. (2) 200-char limit keeps notes concise and prevents the field from dominating the bottom sheet. (3) Notes are user-authored only — the LLM does not write into this field. Implementation: `useRef` + `useEffect` for auto-resize, `maxLength={200}`, `overflow-y-auto` after max height. |
...
| **LLM OUTPUT CONSTRAINT (F3 / F10)** ||||
| D73 | LLM Output — No Descriptions or Commentary | **LLM outputs structured schedule data only (time, activity name, duration, activity type). Task descriptions must be empty.** | The LLM was generating coaching advice and explanations in the `description` field of each routine item (e.g., "Take Fat Boy outside immediately upon waking for his morning potty break. Give him a calm cue like 'go potty' and reward with praise when he finishes."). This clutters the task cards with AI commentary the user didn't ask for, making the timeline feel like a training manual instead of a clean checklist. **New constraint:** The LLM prompt must instruct Claude to return only structured fields (`title`, `scheduled_time`, `duration_minutes`, `activity_type`) with no explanatory text, training tips, behavioral guidance, or coaching advice in any field. The `description` field in `routine_items` should be empty or omitted. Rationale: (1) Users want a scannable daily checklist — not a wall of AI-generated coaching text on every task card. (2) The Notes field (D47) exists for user-authored context; it should not be pre-populated with LLM output. (3) Coaching advice belongs in a dedicated tips/education feature (P2), not embedded in every task. (4) Reduces LLM output tokens per generation (~30-40% fewer tokens when descriptions are omitted), lowering cost and latency. **Implementation:** Update the LLM system prompt in the `generate-routine` Edge Function to explicitly prohibit descriptions. Add a post-processing step that strips any `description` content from the LLM response before writing to `routine_items`. Update `D47` — Notes field for AI-generated tasks is now empty by default (no pre-population). Supersedes the previous assumption in D47 that AI descriptions would pre-populate the Notes field. |
| **WEEKLY ROUTINE EVOLUTION (F14)** ||||
| D74 | Regeneration Schedule | **Every 7 days from last generation (not calendar-based)** | Two options considered: (1) regenerate every Monday at user's local midnight, (2) regenerate every 7 days from the last `generated_at` timestamp. Chose option 2. Rationale: (1) Eliminates the edge case where a user onboards on Sunday and gets a redundant regeneration on Monday — too soon to be useful. (2) Simpler implementation: one comparison (`now > valid_until`) vs. computing "next Monday in user's timezone." (3) Naturally staggers API load across the week — not all users hitting Monday morning simultaneously. (4) Feels more personalized — "your puppy's week" starts when you started, not on an arbitrary calendar day. (5) Avoids timezone complexity entirely — `valid_until` is a UTC timestamp, comparison is straightforward. |
| D75 | Regeneration Trigger | **Lazy client-side check on dashboard load (not server-side cron)** | Three options considered: (1) Supabase `pg_cron` extension runs daily, checks all puppies, triggers edge function for stale routines, (2) external scheduler (e.g., Cloud Scheduler → Edge Function), (3) client checks `valid_until` on dashboard mount and triggers if expired. Chose option 3. Rationale: (1) **Zero cost for inactive users** — no app open means no API call. A cron would generate routines for users who churned weeks ago, wasting LLM credits. (2) **No new infrastructure** — no cron to configure, monitor, or debug. No scheduler service to manage. (3) **The routine is always fresh when the user needs it** — if they open the app, they get the latest. If they don't, we don't pay. (4) A user who disappears for 3 months gets exactly one regeneration on return, not 12 weeks of wasted calls. Trade-off: the user sees the old routine briefly during the ~5-10 second regeneration. This is acceptable and actually a positive UX signal ("the app is updating for my puppy's new week"). |
| D76 | Routine Validity Tracking | **`valid_until` column on `routines` table (not client-side date math)** | Store `valid_until = generated_at + 7 days` as a timestamptz column on the `routines` table, computed at generation time. Client compares `now > routine.valid_until`. Alternative was to do date math client-side (`now - generated_at > 7 days`), but a pre-computed column is cleaner: (1) Single comparison vs. subtraction + comparison. (2) Easy to adjust the regeneration interval server-side later (e.g., 5 days for premium users) by just setting a different `valid_until`. (3) Queryable — can find all expired routines in SQL for analytics or batch cleanup. Null `valid_until` (for legacy routines created before F14) is treated as expired — triggers regeneration on first dashboard load post-deploy. |
| D77 | Regeneration Deduplication | **`regeneration_status` column as lightweight lock** | Multiple household members may open the app around the same time, each triggering the `valid_until` check. Without a lock, both would fire the edge function simultaneously — double API cost. Solution: before calling the edge function, set `regeneration_status = 'in_progress'` on the active routine (guarded by a conditional update: `UPDATE routines SET regeneration_status = 'in_progress' WHERE id = ? AND regeneration_status IS NULL`). The conditional update ensures only one caller wins the race. On completion (success or failure), clear the status back to null. A stale lock older than 5 minutes is treated as expired (edge function crashed or timed out) — the next dashboard load can retry. No separate lock table needed — the column lives on the existing `routines` row. |
| D78 | Completion Context in Prompt | **Aggregate `activity_logs` for past 7 days, pass as structured summary** | The core value prop of F14 is that the routine adapts to what the user actually did — not just the puppy's age. The edge function queries `activity_logs` for the past 7 days, grouped by `activity_type`, computing `completed / total` counts per category. This summary is injected into the user prompt as a new section (e.g., `PAST WEEK COMPLETION: walks 12/14 (86%), training 3/14 (21%), potty 42/42 (100%)`). The system prompt instructs Claude to interpret this: if a category has low completion, consider adjusting timing (not just reducing — maybe the sessions are at inconvenient times). If completion is high, maintain or progress. The LLM decides how to respond — we provide the data, not prescriptive rules. This keeps the prompt simple and leverages the LLM's judgment. |
| D79 | Firebase Overlay Cleanup | **Client-side cleanup after successful regeneration** | When a new routine is generated, the old routine's `routine_item_id`s become orphaned. Firebase documents in `editedRoutineItems` and `deletedRoutineItems` that reference old item IDs become stale overlays. On successful regeneration, a client-side function queries these collections for documents matching the old routine's item IDs and batch-deletes them. Custom user-added tasks in the `tasks` collection are NOT affected — they have no `routine_item_id` reference and persist across generations. Rationale: (1) Client-side is simpler than adding cleanup logic to the edge function (which doesn't have Firebase credentials). (2) Stale overlays don't cause bugs (they reference IDs that no longer match — they're just ignored by the dashboard merge logic). Cleanup is cosmetic/hygiene, not correctness-critical. (3) Batch delete keeps the Firestore operation count low. |
| D80 | Generation Context Snapshot | **`generation_context` JSONB column on `routines` table** | Each new routine stores a snapshot of its generation inputs: `{ age_bracket, age_weeks, completion_summary, schedule_params }`. Rationale: (1) Debugging — if a user reports a bad routine, we can inspect what inputs drove it without re-running the generation. (2) Future "routine evolution history" feature (P2) — show the user how their routine has changed week over week. (3) Analytics — correlate age bracket transitions with retention to validate the F14 hypothesis. (4) JSONB is schemaless — we can add fields later without a migration. The column is nullable (no backfill needed for existing routines). |
| D81 | Edge Function Reuse | **Extend existing `generate-routine`, do not create a new function** | The existing `generate-routine` edge function already handles: auth verification, puppy lookup, `computeScheduleParams()`, Claude API call, validation, routine + routine_items insert. For F14, we extend it to accept an optional `completionContext` parameter. When present, it's injected into the user prompt. When absent (onboarding flow), behavior is identical to before. The `valid_until` and `generation_context` columns are populated at insert time. No new edge function, no new deployment, no new URL. The onboarding flow continues to call the same endpoint — it just doesn't pass `completionContext`. |
| D82 | Regeneration Failure Handling | **Silent failure — old routine stays active, status cleared** | If the Claude API call fails (timeout, rate limit, 500 error), the old routine remains active and fully functional. The `regeneration_status` is cleared so the next dashboard load can retry. No error banner is shown to the user — they simply continue using their existing routine. The brief "Updating..." banner disappears and nothing changes. Rationale: (1) The old routine is still valid and usable — a failed regeneration is not a degraded experience. (2) Showing an error ("Failed to update your routine") would alarm users about something they didn't ask for and can't fix. (3) The lazy trigger naturally retries on next app open — transient API failures resolve themselves. (4) Edge function logs the error server-side for monitoring. Only flag to the user if regeneration fails 3+ times consecutively (P1). |
